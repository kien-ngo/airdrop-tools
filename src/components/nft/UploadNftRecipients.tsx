...
import { NFT_RECIPIENTS_EXAMPLE } from "../../constants/examples";
import {
  closePopup,
  createDownloadLink,
  getCsvDataString,
...
export default function UploadNftRecipients({
  disabled,
  onCompleted,
}: {
  disabled: boolean;
  ...
  const downloadJson = () => {
    const dataStr = getJsonDataString(NFT_RECIPIENTS_EXAMPLE);
    createDownloadLink("downloadExampleFile", dataStr, "example.json");
  };
  const downloadCsv = () => {
    const dataStr = getCsvDataString(NFT_RECIPIENTS_EXAMPLE);
  ...
  const handleJsonUpload = (event: React.ChangeEvent) => {
    try {
      ...
      reader.onload = (event: ProgressEvent<FileReader>) => {
        ...
        try {
          const jsonObject: TNftRecipient[] = JSON.parse(jsonContent);
  ...
  const handleCsvUpload = (event: React.ChangeEvent) => {
    try {
      ...
      reader.onload = (event: ProgressEvent<FileReader>) => {
        ...
        const jsObjects: TNftRecipient[] = csvRows.slice(1).map((row) => {
          const rowValues = row.split(",").map((value) => value.trim());
          const object: TNftRecipient = {
            to: rowValues[0],
            tokenId: Number(rowValues[1]),
          };
